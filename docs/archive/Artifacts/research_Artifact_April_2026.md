...
**Status:** ✅ Section 2.116 Phase JC — Claude Code Figma MCP Setup
...
**Trigger:** Operator instruction: analyze both Figma designs (Admin + Client sides) for COI module, translate design → system architecture, plan Phase 1 development.
**Objective:** Decompose COI requirements, audit existing infrastructure, identify gaps, define data model, plan implementation.
...
**MCP Tool Result:** `400 Bad Request — "File type not supported by this endpoint"` (confirmed on both links)

**Root Cause:** Both URLs use the `figma.com/make` scheme, which generates AI-assisted Figma prototypes. The Figma REST API `/files/{key}` endpoint used by the MCP tool (`mcp_framelink_mcp_get_figma_data`) does NOT support `figma.com/make` file types. Only standard design files at `figma.com/file/{key}` or `figma.com/design/{key}` are accessible.

**Decision:** Design analysis proceeds via codebase reverse-engineering, operator-provided requirements, and CSU institutional standards. MCP design reference is BLOCKED until operator provides a standard Figma file URL.
...
- Controller: `construction-projects.controller.ts` — full CRUD + workflow endpoints
- Service: `construction-projects.service.ts` — business logic, raw SQL queries, ORM operations
- Module: `construction-projects.module.ts`

**Backend entities (5):**

| Entity | Table | Key Fields | Notes |
|--------|-------|-----------|-------|
| `ConstructionProject` | `construction_projects` | `id, projectCode, title, description, campus, status, fundingSourceId, contractorId, contractAmount, contractNumber, startDate, targetCompletionDate, actualCompletionDate, projectDuration, projectEngineer, projectManager, buildingType, floorArea, numberOfFloors, beneficiaries, physicalProgress, financialProgress, publicationStatus, latitude, longitude, idealInfrastructureImage, objectives(jsonb), keyFeatures(jsonb), timelineData(jsonb), galleryImages(jsonb)` | Has `@Filter notDeleted` |
| `ConstructionMilestone` | `construction_milestones` | `id, projectId, title, description, targetDate, actualDate, status, remarks, createdAt` | **Missing `@Filter`** — Directive 285 violation (Phase HL) |
| `ConstructionProjectFinancial` | `construction_project_financials` | `id, projectId, fiscalYear, appropriation, obligation, disbursement, metadata, createdAt` | Has `@Filter notDeleted` |
| `ConstructionGallery` | `construction_gallery` | `id, projectId, imageUrl, caption, category, isFeatured, uploadedAt` | **Missing `@Filter`** — Directive 285 violation (Phase HL) |
| `ConstructionSubcategory` | `construction_subcategories` | (full CRUD module) | Separate module, stable |

**Frontend pages (4):**

| File | Status | Capability |
|------|--------|-----------|
| `pmo-frontend/pages/coi/index.vue` | ✅ Exists | List view, meatball menu, submit/approve/reject/withdraw actions, delete |
| `pmo-frontend/pages/coi/detail-[id].vue` | ✅ Exists | Full detail view, workflow buttons, milestones table, building info, contractor/funding sidebar |
| `pmo-frontend/pages/coi/edit-[id].vue` | ✅ Exists | Edit form with all project fields |
| `pmo-frontend/pages/coi/new.vue` | ✅ Exists | Create form with funding/contractor lookups, staff assignment |

**Backend API endpoints (confirmed in controller):**

| Endpoint | Method | Access |
|----------|--------|--------|
| `/api/construction-projects` | GET | Admin, Staff, Viewer |
| `/api/construction-projects` | POST | Admin, Staff |
| `/api/construction-projects/:id` | GET | Admin, Staff, Viewer |
| `/api/construction-projects/:id` | PATCH | Admin, Staff |
| `/api/construction-projects/:id` | DELETE | Admin |
| `/api/construction-projects/:id/submit-for-review` | POST | Admin, Staff |
| `/api/construction-projects/:id/publish` | POST | Admin |
| `/api/construction-projects/:id/approve` | PATCH | Admin (alias for publish) |
...
| `/api/construction-projects/:id/milestones/:milestoneId` | PATCH, DELETE | Admin, Staff |
| `/api/construction-projects/:id/financials` | GET, POST | GET: all; POST: Admin, Staff |
| `/api/construction-projects/:id/financials/:financialId` | PATCH, DELETE | Admin, Staff |
| `/api/construction-projects/:id/gallery` | GET, POST | GET: all; POST: Admin, Staff |
| `/api/construction-projects/:id/gallery/:galleryId` | GET, PATCH, DELETE | Admin, Staff |

**RBAC:**
- Permission key: `coi` (immutable, in `middleware/permission.ts`)
- Admin: full create/edit/delete/approve
- Staff: create/edit own assignments, submit for review
- Viewer: read-only (non-admin sees PUBLISHED only, or own drafts)
- All COI routes guarded by `JwtAuthGuard + RolesGuard`

---

### JB-C: Design Decomposition (Admin Side — Inferred from Codebase + Requirements)

Since Figma MCP is unavailable, the admin-side COI design is inferred from:
1. Existing frontend implementation (what IS built)
2. Operator requirements (what MUST be built)
3. Analogous modules (UO module as design reference)

#### JB-C-1: Dashboard Overview (KPI Panel — MISSING)

The COI index page currently shows a data table only. The Figma admin design requires a KPI summary panel at the top of the index page. Required stats:

| KPI | Data Source | Notes |
|-----|-------------|-------|
| Total Projects | `COUNT(*)` from `construction_projects` | Group by publication_status |
| Active Projects (ONGOING) | `COUNT(*) WHERE status='ONGOING'` | |
| Completed Projects | `COUNT(*) WHERE status='COMPLETED'` | |
| Total Contract Value | `SUM(contract_amount)` | PHP formatted |
| Average Physical Progress | `AVG(physical_progress)` | % |
| Pending Review | `COUNT(*) WHERE publication_status='PENDING_REVIEW'` | Admin-only KPI |

**Current state:** ABSENT from frontend. Backend `findAll` returns paginated list with no aggregate stats.
**Required:** New endpoint or inline computation from list response.

#### JB-C-2: Project Listing / Table (PARTIALLY COMPLETE)

Current columns: `Project Name, Campus, Status, Publication, Contract Amount, Progress, Actions`

**Gaps vs requirements:**
- No quick-filter chips by status (PLANNING, ONGOING, COMPLETED, etc.)
- No search field on the list page (text search by title/project_code)
- No campus filter dropdown
- No date range filter
- Backend `findAll` supports `status`, `campus`, `contractor_id`, `funding_source_id` query params — filters exist in backend but no frontend UI

#### JB-C-3: Project Details View (PARTIALLY COMPLETE)

Current detail page shows: status/progress header, project info, milestones table, building details sidebar, contractor/funding sidebar.

**Gaps vs requirements:**
1. **Gallery (MOV) section** — backend fully supports gallery CRUD + file upload but `UIProjectDetail` has NO gallery field; `adaptProjectDetail` does NOT map gallery items; detail page has NO gallery section
2. **Financial summary display** — backend `findOne` returns `financials` array but the `BackendFinancial` adapter interface (`description`, `amount`, `financial_type`) does NOT match actual entity columns (`appropriation`, `obligation`, `disbursement`, `fiscal_year`); detail page likely shows empty financial table
3. **Milestone display mismatch** — `adaptMilestone` maps `backend.milestone_name` but ORM entity returns `title`; `weight_percentage` and `progress_percentage` are NOT in the entity — milestones display is broken
4. **Location/map** — `latitude`, `longitude` fields exist in entity but not shown in detail
5. **Approval history/audit trail** — only current publication state shown, no history

#### JB-C-4: Financial Tracking (DATA MISMATCH)

The `ConstructionProjectFinancial` entity has: `appropriation`, `obligation`, `disbursement`, `fiscal_year`.

The frontend adapter `BackendFinancial` expects: `description`, `amount`, `date_recorded`, `financial_type`.

**This is a complete structural mismatch.** The detail page financial table renders empty data because the adapter cannot find the expected fields. The financial display needs to be rebuilt around the actual entity structure.

**Required financial display:**
- Per fiscal year: Appropriation | Obligation | Disbursement | Utilization Rate (obligation/appropriation) | Disbursement Rate (disbursement/obligation)
- This matches the UO Financial Accomplishment module pattern

#### JB-C-5: Milestone Tracking (DATA MISMATCH)

The `ConstructionMilestone` entity has: `title`, `description`, `targetDate`, `actualDate`, `status`, `remarks`.

The frontend adapter `BackendMilestone` expects: `milestone_name`, `target_date`, `actual_date`, `weight_percentage`, `progress_percentage`.

**Mismatches:**
- `milestone_name` → entity has `title` (ORM returns as `title`, not `milestone_name`)
- `weight_percentage`, `progress_percentage` → NOT in entity at all (these fields don't exist)

**Required milestone display:** Title, Description, Target Date, Actual Date, Status, Remarks. Weight/progress fields must be removed (YAGNI — not in schema).

#### JB-C-6: Gallery / MOV (ABSENT)

Backend fully implements gallery:
- `POST /api/construction-projects/:id/gallery` — multipart file upload (10MB limit)
- `GET /api/construction-projects/:id/gallery` — list with `category` filter
- Categories: `PROGRESS`, and potentially `BEFORE`, `AFTER`, `COMPLETION`
- `isFeatured` flag per image

**Frontend gap:** Zero gallery UI. The detail page does not call gallery endpoint. `UIProjectDetail` has no `gallery` property. `adaptProjectDetail` does not include gallery.

**Required:** Gallery section in detail page (image grid), upload dialog (Admin/Staff), delete per item (Admin).

#### JB-C-7: Accomplishment Tracking (NEEDS CLARIFICATION)

`physicalProgress` (decimal 5,2) is a single top-level field on the project. The Figma design requirements mention "accomplishment tracking" which may mean:
- Simply displaying and editing `physicalProgress` as a percentage (already partially done)
- Or a per-milestone weighted progress computation

**Finding:** The entity has no `weightPercentage` or `progressPercentage` per milestone. A single `physicalProgress` on the project is the only accomplishment field. The Phase JB implementation will treat this as: editable `physicalProgress` field on project + per-milestone status tracking (PENDING/IN_PROGRESS/COMPLETED).

---

### JB-D: Design Decomposition (Client Side — New Feature)

The client-side Figma link (`figma.com/make/BLrO1i29yyTrkkg4iyVR5V`) is inaccessible via MCP. Based on the operator's stated requirement of a "transparency-focused, read-only public view" for the Client Side PMO Dashboard:

**Inferred client-side requirements:**
1. Unauthenticated (or separate auth-gated) read-only view of PUBLISHED infrastructure projects
2. Limited to `publication_status = 'PUBLISHED'` projects only
3. No workflow actions (no submit/approve/reject)
4. No administrative fields (no `createdBy`, `submittedBy`, etc.)
5. Public-facing project cards or list: title, campus, status, progress, contractor, funding source, start/end dates
6. Project detail: full non-administrative fields, milestone status (read-only), gallery images, financial summary

**Decision — Scope for Phase JB:**
A client-side public view is a NEW feature requiring:
- A new page at `/coi/public/index.vue` (or `/public/coi/index.vue`) — unauthenticated access pattern
- A new backend endpoint `GET /api/construction-projects/public` (no auth guard) returning only PUBLISHED projects with limited fields
- OR: reuse existing `GET /api/construction-projects` with `publication_status=PUBLISHED` filter and allow unauthenticated access to that specific route

**Risk:** The existing controller wraps ALL routes under `@UseGuards(JwtAuthGuard, RolesGuard)` at controller level. A public endpoint requires either:
1. Separate public controller (`/api/public/construction-projects`) — clean, no guard changes needed
2. `@Public()` decorator override on specific routes — more invasive

**Ruling:** Option 1 (separate public controller) is preferred per STRICT NON-DISRUPTION policy.

---

### JB-E: Data Model Analysis

#### Existing entities — sufficient vs gaps

| Entity | Sufficient for Phase JB? | Gap |
|--------|--------------------------|-----|
| `ConstructionProject` | ✅ Yes | No changes needed |
| `ConstructionMilestone` | ✅ Yes (with adapter fix) | `@Filter` missing (Phase HL) |
| `ConstructionProjectFinancial` | ✅ Yes (with adapter fix) | `@Filter` present |
| `ConstructionGallery` | ✅ Yes | `@Filter` missing (Phase HL); frontend integration absent |
| `ConstructionSubcategory` | ✅ Yes | Not relevant to Phase JB |

**No new entities required for Phase JB.** All required data is covered by existing schema.

**Status history:** A `construction_project_status_history` table was listed in requirements but is NOT in the current schema. Per YAGNI: defer unless operator explicitly requires it for Phase JB.

#### Frontend adapter mismatch summary (critical fixes)

| Interface | Expected field | Actual entity field | Fix |
|-----------|---------------|---------------------|-----|
| `BackendMilestone.milestone_name` | `milestone_name` | `title` | Rename → `title` |
| `BackendMilestone.weight_percentage` | number | NOT in entity | Remove from interface |
| `BackendMilestone.progress_percentage` | number | NOT in entity | Remove from interface |
| `BackendFinancial.description` | string | NOT in entity | Replace with `fiscal_year, appropriation, obligation, disbursement` |
| `BackendFinancial.amount` | number | NOT in entity | Remove |
| `BackendFinancial.financial_type` | string | NOT in entity | Remove |
| `BackendFinancial.date_recorded` | string | NOT in entity | Remove |
| `UIProjectDetail.gallery` | array | NOT in interface | Add |

---

### JB-F: Workflow Analysis

**Existing workflow (confirmed):** `DRAFT → PENDING_REVIEW → PUBLISHED / REJECTED`
...
**Compatibility:** The COI workflow is identical to UO publication lifecycle. No conflict.

**Missing: Withdrawal after rejection.** Currently `withdraw` is only available from `PENDING_REVIEW`. A `REJECTED` project goes back to `DRAFT` status already (per `reject` service logic) — so re-editing is supported without a separate withdraw action.
...
- Permission key: `coi` (in `pmo-frontend/middleware/permission.ts` — immutable)
- `usePermissions().canAdd`, `canEdit`, `canDelete` all gate COI actions in frontend
- Backend enforces: Admin role → full control; Staff → own/assigned records; Viewer → read-only

**OpenLDAP integration:**
- Backend JWT strategy uses `campus` field from user profile for campus-scoped visibility
- Non-admin users with a campus assignment see: (own campus projects) + (PUBLISHED projects) + (own created/assigned)
- COI campus values: `MAIN`, `CABADBARAN`, `BOTH`

**Client-side public view RBAC:** No authentication required — only PUBLISHED records served.

---

### JB-H: UI/UX Alignment

**Confirmed Vuetify patterns in use:**
- `v-data-table` for list and milestones
- `v-card` for sections (project info, building details, contractor/funding)
- `v-chip` for status indicators
- `v-progress-linear` for progress bars
- `v-skeleton-loader` for loading states
- Meatball menu (`v-menu` + `v-list`) for list actions
- `v-dialog` for confirmation dialogs

**CSU branding:**
- Primary color: emerald (`#059669` or configured in Vuetify theme)
- `bg-primary text-white` class on table headers (established in Phase HS)
- `text-h4 font-weight-bold text-grey-darken-3` for page titles

**Gallery/MOV UI pattern to follow:** Vuetify `v-img` grid inside `v-card`, upload via `v-file-input` in `v-dialog`.

---

### JB-I: System Integration Check

| Integration | Status | Notes |
|-------------|--------|-------|
| User Management (`/api/users/eligible-for-assignment`) | ✅ Already integrated | Used in `new.vue` and `edit-[id].vue` for staff assignment |
| Funding Sources (`/api/funding-sources`) | ✅ Already integrated | Used in create/edit forms |
| Contractors (`/api/contractors`) | ✅ Already integrated | Used in create/edit forms |
| `record_assignments` table | ✅ Integrated | CONSTRUCTION module assignments working |
| `pending-reviews` admin page | ✅ Integrated | COI shows as "Infrastructure (COI)" in pending reviews |
| File uploads (UploadsService) | ✅ Backend wired | Gallery endpoint uses `UploadsService` + `FileInterceptor` |
| Analytics endpoints | ❌ Not integrated | COI has no analytics page or aggregated stats endpoint |
| University Operations module | ✅ No conflict | Separate tables, separate service |
| Financial Accomplishment module | ✅ No conflict | COI financials are in `construction_project_financials` (separate table) |

---

### JB-J: Phase JB Scope Summary

**Confirmed in-scope (Phase JB — COI Module Enhancement):**
1. Fix adapter mismatches — `BackendMilestone` and `BackendFinancial` interfaces
2. Fix `adaptMilestone` and `adaptFinancial` functions to match actual entity structure
3. Fix milestone display in detail page (use `title` not `milestone_name`; remove weight/progress columns)
4. Fix financial display in detail page (show `fiscal_year, appropriation, obligation, disbursement, utilization_rate`)
5. Add gallery (MOV) section to detail page — image grid + upload dialog + delete
6. Add KPI stats panel to COI index page (project counts by status, total contract value, average progress)
7. Add filter bar to COI index page (status filter chips, campus dropdown, search field)
8. Add `GET /api/public/construction-projects` endpoint (unauthenticated) returning PUBLISHED projects
9. Add `GET /api/public/construction-projects/:id` endpoint (unauthenticated) returning PUBLISHED project detail
10. Add `/coi/public/index.vue` — unauthenticated read-only list page
11. Add `/coi/public/detail-[id].vue` — unauthenticated read-only detail page

**Deferred (not Phase JB):**
- Status history entity/table (`construction_project_status_history`)
- Advanced analytics/forecasting
- Map/geolocation display (latitude/longitude)
- Timeline Gantt-chart view
- Accomplishment tracking with milestone weighting
- Phase HL `@Filter` fixes (separate phase, already planned)
...
**Scope:** Determine what remains to migrate for the 3 actively-used modules: AuthService, UsersService, UniversityOperationsService — plus any supporting strategies.

---

### Finding JD-A: AuthService — ✅ FULLY MIGRATED

`pmo-backend/src/auth/auth.service.ts` uses `EntityManager` exclusively:
- `em.findOne(User, ...)`, `em.find(UserRole, ...)`, `em.find(Role, ...)`, `em.flush()`, `em.persistAndFlush()`
- No `DatabaseService` import. No `getConnection()` raw SQL.
- Status: **Complete. No work needed.**

---

### Finding JD-B: UsersService — ✅ MIGRATED (Hybrid — Acceptable)

`pmo-backend/src/users/users.service.ts` uses `EntityManager` throughout:
- ORM for: `findOne`, `find`, `create`, `persistAndFlush`, `flush`, `transactional`, `count`
- Raw SQL via `em.getConnection()` for: `findAll()` (complex paginated JOIN), `canModifyUser()` (PG function call), `findEligibleForAssignment()`, `findOne()` join query, bulk operations
- `getActorRank()` — pure ORM (`em.findOne(User)`, `em.findOne(UserRole)`)
- **No `DatabaseService` import.** All raw SQL routed through `em.getConnection()` — valid hybrid pattern per Phase IQ.
- Status: **Complete per IQ hybrid model. No work needed.**

---

### Finding JD-C: UniversityOperationsService — ✅ MIGRATED (Hybrid — Intentional)

`pmo-backend/src/university-operations/university-operations.service.ts`:
- 8 EntityRepositories injected (UO, Indicator, Financial, QR, QRS, FY, Taxonomy, OrgInfo)
- 109 `em.*` ORM calls for all CRUD
- 91 `em.getConnection().execute()` calls for analytics CTEs — intentional per governance (BAR reports require complex raw SQL)
- **No `DatabaseService` import.**
- Status: **Complete. Raw SQL analytics are intentional — not a gap.**

---

### Finding JD-D: LdapStrategy — ⚠️ ONE REMAINING DatabaseService USAGE

`pmo-backend/src/auth/strategies/ldap.strategy.ts`:
- Imports `DatabaseService` and uses `this.db.query()` for a single user lookup:
  ```sql
  SELECT id, email, is_active FROM users WHERE LOWER(email) = LOWER($1) AND deleted_at IS NULL LIMIT 1
  ```
- This is the **only remaining `DatabaseService` usage in all active modules**.
- Fix: Replace with `em.findOne(User, { email: { $ilike: email }, deletedAt: null }, { filters: false })`
- Auth module must inject `EntityManager` into `LdapStrategy` instead of `DatabaseService`
...
| `UniversityOperationsService` | ❌ None | ✅ Complete (hybrid) |
| `LdapStrategy` | ⚠️ 1 query | 🔧 1 fix needed |

**Migration gap: 1 file, 1 query.** Phase JD implementation is minimal.
...
COI analytics will follow the same backend pattern (raw SQL via `em.getConnection().execute()`) and frontend pattern (ApexCharts in `coi/index.vue` analytics tab).
...
|---|---|---|
| `GET /api/construction-projects/analytics/summary` | total, by_status, by_campus, by_publication_status, total_contract_value, avg_progress | Hero KPIs (replaces client-side computation) |
| `GET /api/construction-projects/analytics/campus-breakdown` | per campus: count, total_contract, avg_progress, by_status | Grouped bar chart |
| `GET /api/construction-projects/analytics/status-distribution` | per status: count, total_contract, avg_progress | Donut chart |
| `GET /api/construction-projects/analytics/financial-summary` | total_appropriation, total_obligation, total_disbursement, utilization_rate, disbursement_rate across all projects | Financial hero stats |

All endpoints: no query param required (all projects, all time). Optional `campus` filter on campus-breakdown.

---

### JF-D: Frontend Analytics Layout

**Location:** New "Analytics" tab on `pages/coi/index.vue` (tab toggle: "Projects" | "Analytics").

**Analytics tab contents (top-to-bottom):**

1. **Financial Hero Row** — 4 stat cards: Total Contract Value, Total Appropriation, Total Obligation, Total Disbursement (sourced from financial-summary endpoint)
2. **Status Distribution Donut** (left, md=5) + **Campus Breakdown Bar** (right, md=7)
3. **Progress Distribution** — horizontal bar or gauge showing avg physical progress per campus

**Existing tab:** "Projects" tab = current list view (KPI cards + filter bar + data-table). KPI cards remain on Projects tab (client-side computation — fast, no extra request).

---

### JF-E: Backend Implementation Plan

**File:** `pmo-backend/src/construction-projects/construction-projects.service.ts`

Add 3 methods after `findPublicOne`:
1. `getAnalyticsSummary()` — GROUP BY status + GROUP BY campus + GROUP BY publication_status + SUM/AVG
2. `getCampusBreakdown()` — per campus: count, total_contract_amount, avg_progress, per-status counts
3. `getStatusDistribution()` — per status: count, total_contract_amount, avg_progress

**File:** `pmo-backend/src/construction-projects/construction-projects.controller.ts`

Add 3 GET routes under `/analytics/` prefix. All require auth (Admin/Staff).

**Pattern:** Raw SQL via `em.getConnection().execute()` — consistent with service hybrid model.

---

### JF-F: Scope Boundaries

**In scope:**
- 3 backend analytics endpoints
- Analytics tab on `coi/index.vue` with 3 chart sections
- Financial summary hero row (from financials table aggregation)

**Out of scope (YAGNI):**
- Trend-over-time charts (requires date range slicing)
- Fiscal-year filter (COI financials span multiple years independently)
...
**Status:** Phase 1 Research Complete
**Trigger:** Operator smoke-test of JB Postman gate revealed 20-C and 21-C returning `{data:[], meta:{...}}` (paginated wrapper) instead of plain array. 20-D (404 for nil UUID) is correct behavior — not an error.

### Root Cause

`ConstructionProjectsService.findGallery()` (service line 861–885) returns `PaginatedResponse<ConstructionGallery>` = `createPaginatedResponse(items, total, page, limit)` = `{data: [...], meta: {total, page, limit, totalPages}}`.

Three consumers assumed plain array:
1. **Postman 20-C** — `pm.expect(pm.response.json()).to.be.an('array')` → fails on object
2. **Postman 21-C** — same assertion
3. **`pages/coi/detail-[id].vue` `fetchGallery()`** — `Array.isArray(res) ? res : []` → always `[]`
4. **`pages/coi/public/detail-[id].vue` `fetchGallery()`** — same pattern → always `[]`

### Fix Options

| Option | Change | Risk |
|---|---|---|
| A: Change service to return plain array | Remove `createPaginatedResponse`, return `items` directly | Low; gallery is not paginated in UI anyway |
| B: Fix consumers to unwrap `.data` | Update 2 vue files + 2 Postman tests | Low; preserves service shape for future pagination |

**Decision: Option B** — consumers are the mismatch. Service pagination is consistent with the rest of the codebase. Frontend doesn't use pagination controls on gallery, but keeping the paginated shape is non-breaking and consistent. Fixing consumers is minimal-scope.

### Files to Change

| File | Change |
|---|---|
| `pmo-frontend/pages/coi/detail-[id].vue` | `fetchGallery`: unwrap `res.data` |
| `pmo-frontend/pages/coi/public/detail-[id].vue` | `fetchGallery`: unwrap `res.data` |
| `docs/references/postman-jb-smoke-gate.json` | 20-C and 21-C: assert `body.data` is array |

### 20-D Clarification

`GET /api/public/construction-projects/00000000-0000-0000-0000-000000000000` returning 404 is **correct behavior**. The nil UUID does not exist → `findOne()` throws NotFoundException → `findPublicOne()` propagates it. Test gate expects 404 and passes. No fix needed.
