...
// EEE-E: Supporting Documents per-form workspace. A groupCode-parameterized
// adaptation of CiComplianceRepository for the SD groups (Orders, Reports &
// Monitoring, Certifications & Other Documents). Reuses the existing
// document-checklist + submissions endpoints — no new backend.
...
const groupItems = computed(() =>
  items.value.filter((i) => i.documentType?.groupCode === props.groupCode),
)
const approvedCount = computed(() => groupItems.value.filter((i) => i.submissionStatus === 'APPROVED').length)
const submittedCount = computed(() => groupItems.value.filter((i) => i.submissionStatus !== 'NOT_SUBMITTED').length)
...
// Authenticated download with original filename.
...
async function fetchChecklist() {
  ...
  try {
    ...
    console.error('[CiSupportingDocsRepository] fetch failed:', err)
  } finally {
    loading.value = false
  }
}
...
<template>
  <div>
    <!-- Summary bar -->
    <v-card variant="tonal" :color="color" class="mb-4">
      <v-card-text class="d-flex align-center ga-3 flex-wrap py-2">
        <v-icon :icon="icon" />
        <span class="text-body-2 font-weight-medium">Supporting Documents — {{ groupLabel }}</span>
        <v-spacer />
        <v-chip size="small" variant="elevated" color="success">{{ approvedCount }} approved</v-chip>
        <v-chip size="small" variant="tonal">{{ submittedCount }} / {{ groupItems.length }} submitted</v-chip>
        <v-progress-linear
          :model-value="groupItems.length ? (submittedCount / groupItems.length) * 100 : 0"
    ...
    <div v-if="loading" class="text-center pa-4 text-grey">
      ...
      <div class="mt-2 text-caption">Loading forms…</div>
    </div>

    <div v-else-if="!groupItems.length" class="text-center pa-6 text-grey">
      <v-icon size="48" color="grey-lighten-1">{{ icon }}</v-icon>
      <div class="mt-2">No forms found for this group. Run the latest migration to seed the taxonomy.</div>
    </div>

    <!-- Per-type workspace (two-column responsive grid) -->
    <v-row v-else dense>
      <v-col v-for="item in groupItems" :key="item.id" cols="12" md="6">
        <v-card variant="outlined" class="h-100">
          <v-card-text class="py-2">
            ...
            <!-- Submission metadata -->
            <div v-if="item.submittedAt" class="text-caption text-grey mt-1">
              <v-icon size="x-small" class="mr-1">mdi-clock-outline</v-icon>{{ formatDate(item.submittedAt) }}
