...
async function fetchData() {
  ...
  try {
    ...
    incidentLogRows.value  = monNorm((p as any).incident_log)
...
async function handleSubmit() {
  ...
  try {
    ...
    const payload = {
      ...
      incident_log: incidentLogRows.value,
...
async function uploadRevisionMov() {
  ...
  try {
    if (revisionMovFile.value) {
      ...
      fd.append('description', 'Revision Order MOV')
    ...
    toast.success('Revision Order MOV uploaded')
    ...
    <v-form v-else @submit.prevent="handleSubmit">
      ...
      <v-window v-if="visibleTabOrder.length > 0" v-model="activeTab">
        ...
        <v-window-item v-if="visibleTabOrder.includes('progress')" value="progress">
          ...
          <!-- ── SECTION 2: Milestone Management ── -->
          <div class="d-flex align-center ga-2 mb-1">
            <v-icon icon="mdi-flag-checkered" color="info" size="small" />
            <span class="text-subtitle-1 font-weight-semibold">Milestone Management</span>
          </div>
          <p class="text-body-2 text-grey-darken-1 mb-3">Project phase milestones with start/target dates, progress %, status tracking, and category. Changes are saved immediately.</p>

          <!-- Required order #2: Project Milestones -->
          <v-card variant="outlined" class="mb-4" rounded="lg">
            <v-card-title class="d-flex align-center flex-wrap ga-2 py-2 px-4 bg-grey-lighten-4 text-subtitle-1">
              <v-icon icon="mdi-flag-checkered" size="small" color="info" />
              <span class="font-weight-medium">Milestone Management</span>
              <v-chip size="small" variant="tonal" color="info">{{ existingMilestones.length }}</v-chip>
              <v-spacer />
              <v-btn-toggle
                v-model="milestoneViewMode"
                density="compact"
                variant="outlined"
                divided
                mandatory
            ...
            <v-card-text>
              <v-row dense class="mb-2">
                ...
                <v-col cols="12" sm="6" md="3">
                  <v-select
                    v-model="milestoneEditStatus"
                    :items="[{ title: 'All statuses', value: '' }, ...milestoneStatusOptions]"
                    item-title="title"
                    item-value="value"
                    density="compact"
                    variant="outlined"
                ...
                <v-col cols="12" sm="6" md="2">
                  <v-select
                    v-model="milestoneEditSort"
                    :items="[
                      { title: 'Target Date (Oldest)', value: 'asc' },
                      { title: 'Target Date (Newest)', value: 'desc' },
                    ]"
                    item-title="title"
                    item-value="value"
                    density="compact"
                ...
                <v-col cols="12" md="2" class="d-flex justify-end">
                  <v-btn variant="text" prepend-icon="mdi-refresh" @click="fetchMilestones">Refresh</v-btn>
                </v-col>
              </v-row>

              <v-skeleton-loader v-if="loadingMilestones" type="table" />

              <template v-else-if="editFilteredMilestones.length">

                <!-- ── CARD VIEW ── -->
                <v-row v-if="milestoneViewMode === 'card'" dense>
                  <v-col
                    v-for="milestone in pagedEditMilestones"
                    :key="milestone.id"
                    cols="12"
                    md="6"
                    lg="6"
                  >
                    <v-card variant="outlined" rounded="lg" class="h-100" :style="`border-left: 3px solid rgba(var(--v-theme-${getMilestoneStatusColor(milestone.status)}), 0.7)`">
                      <v-card-text class="py-3">
                        <div class="d-flex align-center ga-2 mb-2">
                          <v-chip size="small" :color="getMilestoneStatusColor(milestone.status)" variant="tonal">{{ milestone.status }}</v-chip>
                          <v-chip v-if="milestone.category" size="x-small" variant="outlined">{{ milestone.category }}</v-chip>
                          <v-spacer />
                          <span class="text-body-2 text-grey">{{ formatMilestoneDate(milestone.targetDate) }}</span>
                          <v-menu v-if="canEditMilestones" location="bottom end">
                            <template #activator="{ props: mp }">
                              <v-btn v-bind="mp" size="x-small" icon="mdi-dots-vertical" variant="text" color="grey-darken-1" />
                            </template>
                            <v-list density="compact" min-width="150">
                              <v-list-item prepend-icon="mdi-pencil" title="Edit milestone" @click="openEditMilestone(milestone)" />
                              <v-list-item v-if="canDeleteResources" prepend-icon="mdi-delete" title="Delete milestone" base-color="error" @click="confirmDeleteMilestone(milestone)" />
                            </v-list>
                          </v-menu>
                        </div>
                        <div class="text-subtitle-2 font-weight-medium mb-2">{{ milestone.title || 'Untitled milestone' }}</div>
                        <div v-if="milestone.description" class="text-body-2 text-grey-darken-1 mb-2">{{ milestone.description }}</div>
                        <div class="d-flex align-center ga-2 mb-1">
                          <v-progress-linear :model-value="milestone.progress ?? 0" :color="(milestone.progress ?? 0) >= 100 ? 'success' : 'primary'" height="6" rounded />
                          <span class="text-body-2 font-weight-medium">{{ milestone.progress ?? 0 }}%</span>
                        </div>
                        <div v-if="milestone.remarks" class="text-caption text-grey mt-1"><strong>Remarks:</strong> {{ milestone.remarks }}</div>
                      </v-card-text>
                    </v-card>
                  </v-col>
                </v-row>
                ...
                <!-- ── LIST VIEW ── -->
                <div v-else-if="milestoneViewMode === 'list'" class="d-flex flex-column ga-2">
                  <v-card
                    v-for="milestone in pagedEditMilestones"
                    :key="milestone.id"
                    elevation="1" border rounded="lg" class="overflow-hidden"
                    :style="`border-left: 3px solid rgba(var(--v-theme-${getMilestoneStatusColor(milestone.status)}), 0.7)`"
                  >
                    <v-row no-gutters align="center" class="pa-3">
                      <v-col cols="12" sm="3" class="d-flex flex-column ga-1 pr-3">
                        <v-chip size="small" :color="getMilestoneStatusColor(milestone.status)" variant="tonal" class="align-self-start">{{ milestone.status }}</v-chip>
                        <v-chip v-if="milestone.category" size="x-small" variant="outlined" class="align-self-start">{{ milestone.category }}</v-chip>
                        <div class="text-body-2 text-grey mt-1">{{ formatMilestoneDate(milestone.targetDate) }}</div>
                      </v-col>
                      <v-col cols="12" sm="7" class="pr-3">
                        <div class="text-subtitle-2 font-weight-medium mb-1">{{ milestone.title || 'Untitled milestone' }}</div>
                        <div v-if="milestone.description" class="text-body-2 text-grey-darken-1 mb-2">{{ milestone.description }}</div>
                        <div class="d-flex align-center ga-2">
                          <v-progress-linear :model-value="milestone.progress ?? 0" :color="(milestone.progress ?? 0) >= 100 ? 'success' : 'primary'" height="6" rounded style="max-width:180px" />
                          <span class="text-body-2 font-weight-medium">{{ milestone.progress ?? 0 }}%</span>
                        </div>
                      </v-col>
                ...
                <!-- ── TABLE VIEW ── -->
                <v-data-table
                  v-else-if="milestoneViewMode === 'table'"
                  :items="editFilteredMilestones"
                  :headers="[
                    { title: 'Title', key: 'title' },
                    { title: 'Category', key: 'category', width: 120 },
                    { title: 'Target Date', key: 'targetDate', width: 120 },
                    { title: 'Actual Date', key: 'actualDate', width: 120 },
                ...
                >
                  <template #item.status="{ item }">
                    <v-chip size="small" :color="getMilestoneStatusColor(item.status)" variant="tonal">{{ item.status }}</v-chip>
                  </template>
                  <template #item.targetDate="{ item }">{{ formatMilestoneDate(item.targetDate) }}</template>
                  <template #item.actualDate="{ item }">{{ item.actualDate ? formatMilestoneDate(item.actualDate) : '—' }}</template>
                  <template #item.category="{ item }">{{ item.category || '—' }}</template>
                  <template #item.progress="{ item }">
                    <div class="d-flex align-center ga-2">
                      <v-progress-linear :model-value="item.progress ?? 0" :color="(item.progress ?? 0) >= 100 ? 'success' : 'primary'" height="6" rounded style="max-width:80px" />
                      <span class="text-body-2">{{ item.progress ?? 0 }}%</span>
                    </div>
                  </template>
                  <template #item.actions="{ item }">
                    <v-menu v-if="canEditMilestones" location="bottom end">
                      ...
                      <v-list density="compact" min-width="150">
                        <v-list-item prepend-icon="mdi-pencil" title="Edit milestone" @click="openEditMilestone(item)" />
                        <v-list-item v-if="canDeleteResources" prepend-icon="mdi-delete" title="Delete milestone" base-color="error" @click="confirmDeleteMilestone(item)" />
                      </v-list>
                    </v-menu>
                  </template>
                </v-data-table>
                ...
                <div v-if="milestoneViewMode !== 'table' && milestoneEditTotalPages > 1" class="d-flex justify-center pt-2">
                  <v-pagination v-model="milestoneEditPage" :length="milestoneEditTotalPages" :total-visible="6" density="comfortable" />
                </div>

              </template>

              <v-alert v-else type="info" variant="tonal" density="comfortable" class="mt-2 mb-0">
                No milestones match the current filter. <span v-if="!existingMilestones.length">Click "Add Milestone" to create the first phase.</span>
              </v-alert>
            </v-card-text>
          </v-card>
        ...
        <!-- ============= TAB 8: OTHERS — Project Monitoring Space (KW-F4) ============= -->
