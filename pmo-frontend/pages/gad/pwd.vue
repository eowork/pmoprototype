...
const form = ref({
  ...
  pwd_category: '',
  subcategory: '',
  total_beneficiaries: null as number | null,
...
const headers = [
  ...
  { title: 'Category', key: 'pwd_category', sortable: true },
  { title: 'Subcategory', key: 'subcategory', sortable: true },
  { title: 'Total', key: 'total_beneficiaries', align: 'center' as const },
...
async function handleSubmit() {
  ...
  try {
    await api.post('/api/gad/pwd-parity', form.value)
    toast.success('PWD parity record created successfully')
    ...
    console.error('[GAD PWD] Failed to create:', err)
  } finally {
...
async function deleteRecord() {
  ...
  deleting.value = true
  try {
    await api.del(`/api/gad/pwd-parity/${recordToDelete.value.id}`)
    ...
    console.error('[GAD PWD] Failed to delete:', err)
  } finally {
...
const filteredRecords = computed(() => {
  ...
  return records.value.filter(
    (r) =>
      ...
      r.pwd_category?.toLowerCase().includes(term)
  )
})
...
async function fetchRecords() {
  try {
    const response = await api.get<{ data: any[] }>('/api/gad/pwd-parity')
    records.value = response.data || []
  } catch (err: unknown) {
    const apiError = err as { message?: string }
    toast.error(apiError.message || 'Failed to load records')
    console.error('[GAD PWD] Failed to fetch:', err)
  } finally {
    loading.value = false
  }
}
...
<template>
  <div>
      ...
      <div class="d-flex align-center ga-3">
        ...
        <div>
          ...
          <p class="text-subtitle-1 text-grey-darken-1">
            Track persons with disability beneficiaries
          </p>
        </div>
      </div>
      ...
      >
        ...
        <template #no-data>
          <div class="text-center pa-6">
            <v-icon icon="mdi-wheelchair-accessibility" size="64" color="grey-lighten-1" class="mb-4" />
    ...
    <v-dialog v-model="createDialog" max-width="600">
      <v-card>
        <v-card-title class="text-h6">Add PWD Parity Record</v-card-title>
        <v-divider />
        <v-form @submit.prevent="handleSubmit">
          <v-card-text>
            <v-row>
              ...
              <v-col cols="12" sm="6">
                <v-text-field v-model="form.pwd_category" label="PWD Category" :rules="[rules.required]" required variant="outlined" density="comfortable" />
              </v-col>
              <v-col cols="12" sm="6">
                <v-text-field v-model="form.subcategory" label="Subcategory" variant="outlined" density="comfortable" />
              </v-col>
              <v-col cols="12" sm="6">
                <v-text-field v-model.number="form.total_beneficiaries" label="Total Beneficiaries" type="number" variant="outlined" density="comfortable" />
              </v-col>
              <v-col cols="12" sm="6">
