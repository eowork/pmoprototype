...
const form = ref({
  ...
  college: '',
  category: '',
  total_faculty: null as number | null,
...
const headers = [
  ...
  { title: 'College', key: 'college', sortable: true },
  { title: 'Category', key: 'category', sortable: true },
  { title: 'Total', key: 'total_faculty', align: 'center' as const },
...
async function handleSubmit() {
  ...
  try {
    await api.post('/api/gad/faculty-parity', form.value)
    toast.success('Faculty parity record created successfully')
    ...
    console.error('[GAD Faculty] Failed to create:', err)
  } finally {
...
async function deleteRecord() {
  ...
  deleting.value = true
  try {
    await api.del(`/api/gad/faculty-parity/${recordToDelete.value.id}`)
    ...
    console.error('[GAD Faculty] Failed to delete:', err)
  } finally {
...
async function fetchRecords() {
  try {
    const response = await api.get<{ data: any[] }>('/api/gad/faculty-parity')
    records.value = response.data || []
  } catch (err: unknown) {
    const apiError = err as { message?: string }
    toast.error(apiError.message || 'Failed to load records')
    console.error('[GAD Faculty] Failed to fetch:', err)
  } finally {
    loading.value = false
  }
}
...
<template>
  <div>
      ...
      <div class="d-flex align-center ga-3">
        ...
        <div>
          ...
          <p class="text-subtitle-1 text-grey-darken-1">
            Monitor faculty gender balance by college
          </p>
        </div>
      </div>
      ...
      >
        ...
        <template #no-data>
          <div class="text-center pa-6">
            <v-icon icon="mdi-human-male-board" size="64" color="grey-lighten-1" class="mb-4" />
    ...
    <v-dialog v-model="createDialog" max-width="600">
      <v-card>
        <v-card-title class="text-h6">Add Faculty Parity Record</v-card-title>
        <v-divider />
        <v-form @submit.prevent="handleSubmit">
          <v-card-text>
            <v-row>
              ...
              <v-col cols="12" sm="6">
                <v-text-field v-model="form.college" label="College" :rules="[rules.required]" required variant="outlined" density="comfortable" />
              </v-col>
              <v-col cols="12" sm="6">
                <v-text-field v-model="form.category" label="Category" :rules="[rules.required]" required variant="outlined" density="comfortable" />
              </v-col>
              <v-col cols="12" sm="6">
                <v-text-field v-model.number="form.total_faculty" label="Total Faculty" type="number" variant="outlined" density="comfortable" />
              </v-col>
              <v-col cols="12" sm="6">
