...
## Schema Coverage vs Main Modules (2025-12-16 Snapshot)

**Target file**: `prototype/database draft/Latest db schema/2025-12-16/PMODataFirebird_Enhanced_postgres.sql`  
**Goal**: Verify which major functional modules already have concrete tables in the dated PostgreSQL schema, and where gaps remain. This prevents duplicating tables in Phase 2 and keeps the plan aligned with reality.
...
- **Present in schema**:
  ...
  - Cross-cutting support tables: `notifications`, `system_settings`, `audit_trail`
- **Not yet implemented** (exists only as design in this research document / plan):
  - `role_type_enum`
  - `user_roles.is_superadmin` and related audit columns
  - `user_page_permissions`
  - `university_operations_personnel`
  - `construction_project_assignments`
  - Helper functions: `is_user_superadmin()`, `get_user_page_permission()`
  - RLS policies and permission-assignment trigger, `permission_audit_log`

**Conclusion**: Core RBAC + user management exist, but the refined, granular RBAC Option 1 schema is still **to be added** in Phase 2/3.
...
- **Present**:
  ...
  - Enhanced `operation_indicators` (Target vs Actual + quarterly targets/accomplishments, scores, UACS, variance)
  - Enhanced `operation_financials` (quarterly financials, utilization, variance, status, program/department/budget_source)
- **Gap**:
  - No additional tables beyond these are required for core Target vs Actual and Financial tabs based on current UI mappings.

**Conclusion**: University Operations is **well covered**; Phase 2 work here is limited to RBAC integration, not new core tables.

### 3. Construction of Infrastructure (COI)

- **Present**:
  - `projects` (generic project backbone with `project_type_enum`)
  - `construction_projects`
  - `construction_project_financials`
  - `construction_project_progress`
  - `construction_pow_items`
  - Shared polymorphic tables: `documents`, `media`
- **Missing vs COI project-detail UI**:
  - New per-tab tables from COI analysis:
  ...
  - Additional columns on existing tables:
    - `construction_projects`: ideal image, beneficiaries, objectives, key_features, original_contract_duration
    - `construction_pow_items`: estimated material/labor cost, date_of_entry, status, remarks, is_unit_cost_overridden
    - `media` / `documents`: gallery- and document-specific metadata (thumbnail, dimensions, tags, versioning, processing status, etc.)

**Conclusion**: COI core is present, but **detailed project‑detail support remains to be implemented** via new tables and ALTERs in Phase 2.

### 4. GAD Parity Report

- **Present**: None of the `gad_*` tables appear in the 2025-12-16 schema.
  - No `gad_student_parity_data`, `gad_faculty_parity_data`, `gad_staff_parity_data`, `gad_pwd_parity_data`, `gad_indigenous_parity_data`
  - No `gad_gpb_accomplishments`, `gad_budget_plans`

**Conclusion**: The GAD Parity module currently has **no dedicated database tables** in the dated schema; all 7 proposed tables remain to be created.

### 5. Repairs Category

- **Present**:
  - `repair_types`
  - `repair_projects` (core repair project data)
  - `repair_pow_items` (POW items per repair project)
- **Already implemented vs earlier research assumption**:
  - Earlier research treated `repair_projects` and `repair_pow_items` as “to be created”; the 2025-12-16 schema already defines both with rich fields.
- **Remaining gaps vs Repairs detail UI**:
  - Per-tab tables analogous to COI:
  ...
  - Extra columns on `repair_pow_items`:
    - Status, remarks, is_unit_cost_overridden (currently not in SQL file)

**Conclusion**: Repairs **core objects already exist**; Phase 2 work is to add the per‑tab detail tables and extend `repair_pow_items`, not to recreate core repair project tables.

### 6. Policies

- **Present**:
  - No explicit `policies` or `policy_*` tables in the 2025-12-16 schema.
  - Only generic support that could be reused:
    - `documents` (policy files)
    - `system_settings` (global flags or validity settings)
- **Gap**:
  - Whatever structures are required by the Policies UI (e.g., `RefinedValidityPoliciesPage`, category/grouping, validity windows, status) are **not modeled as first-class tables** yet.

**Conclusion**: Policies are **not yet represented** as a dedicated module in the schema; separate research and design will be needed before planning DDL.

### 7. Classroom & Administrative Offices (Facilities Assessment)
...
- **Present**:
  ...
  - `room_assessments`
- **Coverage vs UI**:
  - These tables already capture:
    - Campus/building/room metadata (including capacity, type, accessibility, etc.)
    - Assessment data per room:
      - academic_year, semester, subject info, number_of_students, schedule
      - component scores (functionality, utility systems, sanitation, equipment, furniture, space management, safety, overall)
      - detailed JSON fields per component, assessor info, remarks, recommended_actions

**Conclusion**: The Facilities Assessment module (classrooms & admin offices) is **already well represented** in the 2025-12-16 schema; any additional gaps should be validated directly against the UI, but no new core tables are obviously required.

### 8. Forms Module

- **Present**:
  - No dedicated “forms inventory” or “forms catalog” tables appear in the schema.
  - Closest reusables:
    - `documents` (for uploaded templates/PDFs)
    - `system_settings` (for config flags)
- **Gap**:
  - The Forms pages (`ImprovedFormsOverview`, `EnhancedFormsInventory`, etc.) imply:
    - A list of forms, with metadata (code, title, owner, category, storage location, validity, status)
    - Possibly versioning and linkage to documents
  - None of this exists as a first-class schema yet.

**Conclusion**: The Forms module is **not yet modeled** in the PostgreSQL schema; it will need its own design and DDL in a future phase.
