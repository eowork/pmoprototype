...
import { AddMilestoneDialog, EditMilestoneDialog, TimelineMilestone } from '../../dialogs/TimelineDialogs';
...
export function TimelineTabEnhanced({ projectId, canAdd, canEdit, canDelete }: TimelineTabEnhancedProps) {
  const [milestones, setMilestones] = useState<TimelineMilestone[]>([]);
  const [showAddDialog, setShowAddDialog] = useState(false);
  const [editingMilestone, setEditingMilestone] = useState<TimelineMilestone | null>(null);
  const [deletingMilestone, setDeletingMilestone] = useState<TimelineMilestone | null>(null);
  ...
  useEffect(() => {
    const sampleMilestones: TimelineMilestone[] = [
      {
        id: 'MS-001',
        title: 'Site Preparation & Foundation',
        description: 'Site clearing, excavation, foundation work, and structural framework completed ahead of schedule',
        startDate: '2023-06-01',
        endDate: '2023-09-14',
        actualStartDate: '2023-06-01',
        actualEndDate: '2023-09-13',
        status: 'Completed',
        progress: 100,
        durationDays: 106,
        varianceDays: 1
      },
      {
        ...
        description: 'Steel framework installation, roof system completion, window installation, and MEP rough-in underway',
      ...
      {
        id: 'MS-003',
        title: 'MEPF & Finishing Works',
        description: 'Complete mechanical, electrical, plumbing, and fire protection systems; interior finishing; quality assurance',
      ...
      {
        id: 'MS-004',
        title: 'Exterior Finishing & Landscaping',
        description: 'Exterior wall finishing, painting, landscaping, and site development works',
      ...
      },
      {
        ...
        description: 'System testing, commissioning of all MEP systems, and final inspections',
      ...
      },
      {
        ...
        description: 'Final documentation, as-built drawings, and formal project handover to client',
  ...
  // CRUD Handlers
  const handleAddMilestone = (data: Omit<TimelineMilestone, 'id'>) => {
  ...
  const handleEditMilestone = (data: TimelineMilestone) => {
  ...
  const getStatusBadgeColor = (status: TimelineMilestone['status']) => {
    switch (status) {
      case 'Completed':
