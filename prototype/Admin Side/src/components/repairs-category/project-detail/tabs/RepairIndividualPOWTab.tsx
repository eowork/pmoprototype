/**
 * Repair Individual POW Tab - Exclusive for Repairs Category
 * Program of Works management for repair projects
 * 
...
import { formatCurrency, formatDate } from '../../../construction-infrastructure/utils/analyticsHelpers';
import { toast } from 'sonner@2.0.3';
import { AddRepairPOWItemDialog, EditRepairPOWItemDialog, RepairPOWItem } from '../../dialogs/RepairPOWDialogs';

interface RepairIndividualPOWTabProps {
  projectId: string;
  userRole: string;
  canAdd: boolean;
  canEdit: boolean;
  canDelete: boolean;
  powItems?: RepairPOWItem[];
  onPowItemsChange?: (items: RepairPOWItem[]) => void;
}
...
export function RepairIndividualPOWTab({ 
  projectId, 
  userRole, 
  canAdd, 
  canEdit, 
  ...
  onPowItemsChange
}: RepairIndividualPOWTabProps) {
  const [powItems, setPowItems] = useState<RepairPOWItem[]>(externalPowItems || []);
  const [searchTerm, setSearchTerm] = useState('');
  const [showAddDialog, setShowAddDialog] = useState(false);
  const [editingItem, setEditingItem] = useState<RepairPOWItem | null>(null);
  const [deletingItem, setDeletingItem] = useState<RepairPOWItem | null>(null);
  ...
  useEffect(() => {
    const sampleData: RepairPOWItem[] = [
      {
        id: 'RPOW-001',
        description: 'Initial Facility Assessment & Damage Evaluation',
        quantity: 1,
        unit: 'lot',
        estimatedMaterialCost: 5000,
        estimatedLaborCost: 15000,
        estimatedProjectCost: 20000,
        unitCost: 20000,
        dateOfEntry: '2024-01-05',
        status: 'Completed'
      },
      {
        id: 'RPOW-002',
        description: 'HVAC System Repair and Replacement',
        quantity: 8,
        unit: 'units',
        estimatedMaterialCost: 320000,
        estimatedLaborCost: 180000,
      ...
      {
        id: 'RPOW-003',
        description: 'Electrical Wiring & Panel Upgrades',
        quantity: 1,
        unit: 'lot',
      ...
      {
        id: 'RPOW-004',
        description: 'Plumbing System Repairs - Pipe Replacement',
        quantity: 150,
        unit: 'linear meters',
        estimatedMaterialCost: 90000,
        estimatedLaborCost: 60000,
      ...
      {
        id: 'RPOW-005',
        description: 'Ceiling & Wall Repair Works',
        quantity: 250,
        unit: 'square meters',
        estimatedMaterialCost: 75000,
      ...
      {
        id: 'RPOW-006',
        description: 'Floor Tile Repair & Replacement',
        quantity: 180,
        unit: 'square meters',
        estimatedMaterialCost: 108000,
        estimatedLaborCost: 72000,
      ...
      {
        id: 'RPOW-007',
        description: 'Window & Door Repair Works',
        quantity: 25,
        unit: 'units',
        estimatedMaterialCost: 125000,
        estimatedLaborCost: 75000,
  ...
  // CRUD Handlers
  const handleAddItem = (data: Omit<RepairPOWItem, 'id'>) => {
    const newItem: RepairPOWItem = {
      ...data,
      id: `RPOW-${String(powItems.length + 1).padStart(3, '0')}`
    };
    setPowItems([...powItems, newItem]);
  };

  const handleEditItem = (data: RepairPOWItem) => {
  ...
  const handleDeleteItem = () => {
    if (deletingItem) {
      ...
      toast.success(`Repair POW item ${deletingItem.id} deleted successfully`);
  ...
  const handleExportData = () => {
    const csv = [
      ...
      ...filteredAndSearchedItems.map(item => 
        columns.filter(c => c.visible).map(c => {
          const value = item[c.id as keyof RepairPOWItem];
    ...
    a.href = url;
    a.download = `Repair_POW_Items_${projectId}_${new Date().toISOString().split('T')[0]}.csv`;
    a.click();
    toast.success('Repair POW data exported successfully');
  };
  ...
  return (
    <div className="space-y-6">
          ...
          <div className="flex items-center justify-between">
            <div>
              <CardTitle className="flex items-center gap-2">
                ...
                Individual Program of Works (Repair)
              </CardTitle>
              <CardDescription>Detailed breakdown of repair work items with cost estimates</CardDescription>
            </div>
        ...
        <CardContent className="p-6">
                ...
                <TableBody>
                  {paginatedItems.length > 0 ? (
                    ...
                    <TableRow>
                      ...
                      >
                        No repair POW items found matching your filters
                      </TableCell>
                    </TableRow>
                  )}
                </TableBody>
          ...
          <div className="mt-6 grid grid-cols-3 gap-4">
            ...
            <div className="bg-gray-900 text-white rounded-lg p-4">
              <p className="text-xs text-gray-300 mb-1">Total Repair Project Cost</p>
      ...
      {/* Dialogs */}
      <AddRepairPOWItemDialog
        open={showAddDialog}
        onOpenChange={setShowAddDialog}
        onSubmit={handleAddItem}
      />

      <EditRepairPOWItemDialog
        open={!!editingItem}
        ...
        <AlertDialogContent>
          <AlertDialogHeader>
            ...
            <AlertDialogDescription>
              Are you sure you want to delete repair POW item "{deletingItem?.id}"? This action cannot be undone.
            </AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>
