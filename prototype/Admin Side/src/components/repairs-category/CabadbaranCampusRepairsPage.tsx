...
interface CabadbaranCampusRepairsPageProps {
  category: string;
...
export function CabadbaranCampusRepairsPage({
  category,
  onProjectSelect,
  userRole,
  filterData,
  requireAuth,
  onClearFilters
}: CabadbaranCampusRepairsPageProps) {
  ...
  useEffect(() => {
    setLoading(true);
    try {
      // Filter projects for Cabadbaran Campus only
      const cabadbaranProjects = cabadbaranCampusRepairsData.filter(
        project => project.campus === 'Cabadbaran Campus' || project.campus === 'BXU'
      );
      setProjects(cabadbaranProjects);
    } catch (error) {
      console.error('Error loading Cabadbaran Campus repairs data:', error);
    ...
    if (editingProject) {
      ...
      setProjects(projects.map(p => 
        p.id === editingProject.id 
          ? { ...p, ...projectData, campus: 'Cabadbaran Campus' }
          : p
      ));
  ...
  const handleExportData = () => {
    try {
      ...
      link.href = url;
      link.download = `cabadbaran-campus-repairs-${new Date().getFullYear()}.json`;
      link.click();
      ...
      <div className="max-w-7xl mx-auto p-6 space-y-6">
        ...
        <div className="flex flex-col space-y-4">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h1 className="text-2xl font-bold">Cabadbaran Campus Repairs</h1>
              <p className="text-muted-foreground">
                Caraga State University - Cabadbaran Campus repair and maintenance projects
              </p>
            </div>
          </div>
        ...
        {isFormOpen && (
          <RepairProjectFormDialog
            ...
            defaultCampus="Cabadbaran Campus"
          />
        )}
      </div>
    </div>
