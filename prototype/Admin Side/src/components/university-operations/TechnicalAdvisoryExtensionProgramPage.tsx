...
import { Users, Edit, Grid3X3, List, Table, Building2, Database, Shield } from 'lucide-react';
import { toast } from 'sonner@2.0.3';
import { 
  ExtensionAssessment, 
  FIXED_EXTENSION_INDICATORS,
  AnalyticsView,
  OrganizationalInfo,
  calculateAverage, 
  calculateVariance
} from './types/ExtensionAssessment';
import { 
  generateExtensionAssessmentDataByYear,
  generateDefaultOrganizationalInfo,
  validateOrganizationalInfo,
  getAvailableYears,
  calculateCommunityImpact
} from './utils/extensionAssessmentData';
...
interface TechnicalAdvisoryExtensionProgramPageProps {
  category: string;
...
export function TechnicalAdvisoryExtensionProgramPage({
  category,
  onProjectSelect,
  userRole,
  filterData,
...
}: TechnicalAdvisoryExtensionProgramPageProps) {
  const [selectedYear, setSelectedYear] = useState(new Date().getFullYear());
  const [assessmentData, setAssessmentData] = useState<ExtensionAssessment[]>([]);
  ...
  const [selectedIndicator, setSelectedIndicator] = useState<ExtensionAssessment | null>(null);
  const [formData, setFormData] = useState<Partial<ExtensionAssessment>>({});
  ...
  useEffect(() => {
    const data = generateExtensionAssessmentDataByYear(selectedYear);
  ...
  const overallStats = {
    ...
    averageAchievement: assessmentData.length > 0 
      ...
      : 0,
    ...calculateCommunityImpact(assessmentData)
  };
  ...
  // Enhanced validation for extension-specific data
  const validateExtensionData = (data: Partial<ExtensionAssessment>): string[] => {
    const errors: string[] = [];
    ...
    if (data.beneficiaries !== undefined && data.beneficiaries < 0) {
      errors.push('Beneficiaries count cannot be negative');
    }

    if (data.communityProjects !== undefined && data.communityProjects < 0) {
      errors.push('Community Projects count cannot be negative');
    }

    if (data.trainingsProvided !== undefined && data.trainingsProvided < 0) {
      errors.push('Trainings Provided count cannot be negative');
    }

    if (data.practitioners !== undefined && data.practitioners < 0) {
      errors.push('Practitioners count cannot be negative');
    }

    if (data.impactScore !== undefined && (data.impactScore < 0 || data.impactScore > 100)) {
  ...
  const handleEditIndicator = () => {
    if (!requireAuth('edit extension assessment records') || !selectedIndicator) return;
    
    const errors = validateExtensionData(formData);
    if (errors.length > 0) {
      toast.error('Please fix the following errors: ' + errors.join(', '));
      return;
    }

    const updatedIndicator: ExtensionAssessment = {
      ...selectedIndicator,
      ...
      beneficiaries: typeof formData.beneficiaries === 'number' ? formData.beneficiaries : selectedIndicator.beneficiaries,
      communityProjects: typeof formData.communityProjects === 'number' ? formData.communityProjects : selectedIndicator.communityProjects,
      trainingsProvided: typeof formData.trainingsProvided === 'number' ? formData.trainingsProvided : selectedIndicator.trainingsProvided,
      practitioners: typeof formData.practitioners === 'number' ? formData.practitioners : selectedIndicator.practitioners,
      impactScore: typeof formData.impactScore === 'number' ? formData.impactScore : selectedIndicator.impactScore,
      communityPartners: formData.communityPartners || selectedIndicator.communityPartners,
      extensionArea: formData.extensionArea || selectedIndicator.extensionArea,
      targetSector: formData.targetSector || selectedIndicator.targetSector,
      updatedAt: new Date()
    };
    ...
    setFormData({});
    toast.success('Extension assessment record updated successfully');
  };

  const handleDeleteIndicator = (indicator: ExtensionAssessment) => {
    if (!requireAuth('delete extension assessment records')) return;
    ...
    if (confirm(`Are you sure you want to delete "${indicator.particular}"? This action cannot be undone.`)) {
      ...
      toast.success('Extension assessment record deleted successfully');
    }
  };
  ...
  const openEditDialog = (indicator: ExtensionAssessment) => {
    ...
    setFormData({
      ...
      beneficiaries: indicator.beneficiaries,
      communityProjects: indicator.communityProjects,
      trainingsProvided: indicator.trainingsProvided,
      practitioners: indicator.practitioners,
      impactScore: indicator.impactScore,
      communityPartners: indicator.communityPartners,
      extensionArea: indicator.extensionArea,
      targetSector: indicator.targetSector
    });
      ...
      <div className="max-w-7xl mx-auto p-6 md:p-8 space-y-6">
          ...
          <div className="p-6 space-y-4">
            <div className="flex items-start gap-4">
              ...
              <div className="flex-1">
                <h1 className="text-gray-900 text-2xl mb-1">Technical Advisory Extension Program</h1>
                <p className="text-base text-gray-600 leading-relaxed">
                  Quarterly extension assessment and community impact monitoring system
                </p>
              </div>
            </div>
            ...
            <div className="flex items-center gap-3 pt-4 border-t border-gray-100">
              <div className="flex items-center gap-2 px-4 py-2 bg-purple-50 rounded-lg border border-purple-100">
                ...
                <span className="text-lg text-purple-600">{selectedYear}</span>
              </div>
        ...
        <Tabs value={activeTab} onValueChange={setActiveTab} className="space-y-6">
            ...
            <Card className="admin-card border-0">
                ...
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    ...
                    <CardDescription className="text-sm mt-1">
                      Comprehensive data visualization and insights from extension assessment data
                    </CardDescription>
                  </div>
                </div>
              </CardHeader>
              <CardContent className="p-6">
                ...
                <AnalyticsDashboard 
                  ...
                  FIXED_OUTCOME_INDICATORS={FIXED_EXTENSION_INDICATORS}
          ...
          <TabsContent value="target-vs-actual" className="space-y-4 sm:space-y-6">
                ...
                <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
                  <div className="flex-1 min-w-0">
                    ...
                    <CardDescription className="text-sm sm:text-base">
                      Department and entity details for extension assessment reporting
                    </CardDescription>
                  </div>
                  {canEdit && (
                    <Button 
            ...
            <Card className="bg-gradient-to-r from-purple-50 to-violet-100 border border-purple-300 shadow-sm">
              <CardContent className="p-4 sm:p-6">
                <div className="text-center">
                  <h3 className="text-sm sm:text-base lg:text-lg font-semibold text-purple-900 leading-relaxed">
            ...
            <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
              <div className="flex-1 min-w-0">
                <h2 className="text-lg sm:text-xl font-semibold text-gray-900">Extension Outcome Indicators - {selectedYear}</h2>
                <p className="text-sm text-slate-600">Standardized extension indicators with quarterly performance data</p>
              </div>
              <div className="flex items-center gap-1 border border-slate-300 rounded-md p-1 bg-white shadow-sm">
                <Button
                  ...
                  className="data-[state=active]:bg-purple-100 p-2"
                >
            ...
            {/* Enhanced Data Collection Views with Extension-specific criteria and responsive design */}
            <DataCollectionViews
              assessmentData={assessmentData}
              FIXED_OUTCOME_INDICATORS={FIXED_EXTENSION_INDICATORS}
        ...
        <Dialog open={isOrgEditDialogOpen} onOpenChange={setIsOrgEditDialogOpen}>
          <DialogContent className="max-w-[95vw] sm:max-w-2xl bg-white border-0 shadow-xl">
            <DialogHeader className="bg-gradient-to-r from-purple-800 to-violet-800 text-white -m-6 p-4 sm:p-6 mb-0">
              ...
              <DialogDescription className="text-purple-100 mt-2 text-sm sm:text-base">
                Update organizational details for the Technical Advisory Extension Program
              </DialogDescription>
            </DialogHeader>
            ...
            <DialogFooter className="bg-slate-100 border-t border-slate-300 -m-6 mt-0 p-4 sm:p-6">
              <div className="flex flex-col sm:flex-row justify-between w-full gap-3 sm:gap-0">
                ...
                <Button onClick={handleEditOrganizationalInfo} className="bg-purple-600 hover:bg-purple-700 order-1 sm:order-2">
