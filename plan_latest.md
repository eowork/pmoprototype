...
This section defines what DDL we will add (and where) in the main PostgreSQL schema file to support the Construction of Infrastructure (COI) detail pages, GAD Parity module, and Repairs module, following KISS/YAGNI and the greenfield strategy above. It is aligned with the **actual** contents of `2025-12-16/PMODataFirebird_Enhanced_postgres.sql` as documented in `research_latest.md` (Schema Coverage vs Main Modules).
...
**Goal**: Align the **existing** `repair_projects` / `repair_pow_items` tables and new repair‑specific tables with the Repairs UI (Overview, Timeline, POW, Data Analytics, Gallery, Documents, Team), leveraging patterns reused from COI **without duplicating already-defined core tables**.

- **Existing repairs tables already present (keep as-is for core shape)**:
  - `repair_projects` (already in SQL snapshot; provides core repair project data tied to generic `projects` via FK).
  - `repair_pow_items` (already in SQL snapshot; contains item_number, description, quantity, estimated_material_cost, estimated_labor_cost, estimated_project_cost, unit_cost, date_entry, category, phase).

- **ALTER plan for `repair_pow_items` only (extend, do not recreate)**:
  - `ALTER TABLE IF EXISTS repair_pow_items ADD COLUMN IF NOT EXISTS ...` for:
