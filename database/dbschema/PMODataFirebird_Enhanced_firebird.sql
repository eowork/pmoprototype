...
CREATE DOMAIN DEPARTMENT_STATUS_ENUM AS VARCHAR(10) DEFAULT 'ACTIVE'
...
CREATE TABLE departments (
  ...
  email VARCHAR(255),
  phone VARCHAR(20),
  status DEPARTMENT_STATUS_ENUM NOT NULL,
...
-- Triggers to ensure timestamps for departments
SET TERM ^^ ;
...
CREATE TRIGGER departments_bu FOR departments
...
CREATE TABLE media (
  ...
  is_featured SMALLINT DEFAULT 0 CHECK (is_featured IN (0,1)),
  uploaded_by UUID_STR NOT NULL REFERENCES users(id),
  created_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP NOT NULL,
  updated_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP NOT NULL
);
...
CREATE TABLE system_settings (
  ...
  is_public SMALLINT DEFAULT 0 CHECK (is_public IN (0,1)),
  description TEXT_BLOB,
  updated_by UUID_STR REFERENCES users(id),
  created_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP NOT NULL,
  updated_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP NOT NULL
...
CREATE TABLE rooms (
  ...
  is_airconditioned SMALLINT DEFAULT 0 CHECK (is_airconditioned IN (0,1)),
  has_projector SMALLINT DEFAULT 0 CHECK (has_projector IN (0,1)),
  has_whiteboard SMALLINT DEFAULT 1 CHECK (has_whiteboard IN (0,1)),
  is_wheelchair_accessible SMALLINT DEFAULT 0 CHECK (is_wheelchair_accessible IN (0,1)),
  created_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP NOT NULL,
  updated_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP NOT NULL
);
...
CREATE TABLE repair_projects (
  ...
  is_emergency SMALLINT DEFAULT 0 NOT NULL CHECK (is_emergency IN (0,1)),
  campus CAMPUS_ENUM NOT NULL,
...
CREATE TABLE construction_pow_items (
  ...
  phase VARCHAR(100),
  is_variation_order SMALLINT DEFAULT 0 NOT NULL CHECK (is_variation_order IN (0,1)),
